...
class ev_model:
    def __init__(self,input_dim, output_dim, xtrain, ytrain, xtest, ytest):
    ...
    def parallel(self, threads = 12):
        ...
        results = pool.map(self.run_models, )
        pool.close()
        pool.join()
        return results
    '''
    def pool_run(self):
        model = self.build_model(ar)
        history = self.model.fit(self.xtrain, self.ytrain, validation_data=(self.xtest, self.ytest),
                                     epochs=self.epochs, verbose = 0, batch_size=ar[0])
        self.ev_scores.update({self.ev_cycle: {pop_d}})
