...
class ev_model:
    def __init__(self,results):
        ...
        self.path = '/accts/msweber/Code/Boston'
        self.model = None
        self.lr = 0.01
        self.decay = 0.1
        self.pop = {}
    ...
    def score_pop(self):
        for i in range(self.pop_size):
            self.build_models(self.pop[i])
    
    
    def build_models(self, model_array):
        filesToDelete = []
        for file in os.listdir(self.path):
            if file.endswith('h5') or file.endswith('pickel') or file.endswith('json') or file.endswith('scr'):
        ...
        with open(self.path + str(i) + '.json', 'w') as jf:
            jf.write(model_json)
            model.model.save_weights(self.path + str(i) + '.h5')
        while len(models) < self.pop_size:
            time.sleep(1)
        mls = []
        mls2 = []
        for i in range(self.pop_size):
            ml = (models[i], i)
            ml2 = (models[i], i, self.pop[i][0], self.pop[i][2], self.pop[i][1])
            mls.append(ml)
            mls2.append(ml2)
        pool = ThreadPool(processes = len(mls2))
        results = pool.map(self.run, mls2)
        while len(results) < len(mls2):
            time.sleep(1)
        print(results)
        
    
    def fit(self, batch_inputs, batch_targets, batch_size, epochs):
        history = self.model.fit(self.xtrain, self.ytrain, validation_data=(self.xtest, self.ytest),
                                 batch_size=model_array[0], epochs=self.epochs, verbose = 1)
        return history.history['val_acc']
        

                
    def evaluate_individual(self, ml2):
        model, index, batch_size, otimizer, loss = ml2
        if model == None:
            return {index:-1}
        #interop = KerasInterop(self.xtrain, self.ytrain, self.xtest, self.ytest, self.epochs, self.path)
    ...
    def run(self, ml2):
        model, index, batch_size, otimizer, loss = ml2
        data = (self.xtrain, self.ytrain, self.xtest, self.ytest, batch_size, self.epochs, 
                self.path + '/model' + str(index) + '.h5', self.optimizer, self.loss)
        print(data)
        fileName = self.path + '/modelData' + str(index) + '.pickel'
        print(fileName)
        with open(fileName, 'wb') as f:
            pickle.dump(data, f)
        os.system(r'/usr/lfs/v0/anaconda3/envs/tfgpu/bin/python' + self.path +
                  '/KerasThreadRunner.py' + self.path + '/modelData' + str(index) + '.pickel')
        while not os.path.exists(fileName.replace('modelData', 'history').replace('.pickel', '.scr')):
            time.sleep(1)
        try:
            with open(fileName.replace('modelData', 'history').replace('.pickel', '.scr'), 'r') as f:
                history = f.read()
                print(history)
                return history
